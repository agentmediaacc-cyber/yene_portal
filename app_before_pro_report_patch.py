...
def inject_admin_report_center_button(response):
    try:
        ...
        if "/admin/report-center" in body:
            return response
...
def api_admin_weekly_agent_report_pdf():
    ...
    def _row_name(r):
        return str(_pick(r, "full_name", "name", "driver_name", "client_name")).strip()
    ...
    for code_norm, agent in agents_map.items():
        report[code_norm] = {
            "agent": agent,
            "code": _agent_code(agent),
            "agent_name": _agent_name(agent),
            "phone": _agent_phone(agent),
            "drivers": [],
            "clients": [],
            "driver_days": [0, 0, 0, 0, 0, 0, 0],
            "client_days": [0, 0, 0, 0, 0, 0, 0],
        }
    ...
    for r in weekly_drivers:
        ...
        di = _day_idx(_pick(r, "created_at", "registered_at", "signup_date"))
        if di is not None:
            bucket["driver_days"][di] += 1
    ...
    if selected_agent_code:
        rows = [r for r in rows if _norm(r["code"]) == _norm(selected_agent_code)]

    rows.sort(key=lambda x: ((x["agent_name"] or "").lower(), (x["code"] or "").lower()))

    tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".pdf")
    c = canvas.Canvas(tmp.name, pagesize=landscape(A4))
    ...
    def header():
        ...
        c.drawString(left, y, "YENE Weekly Agent Report")
        c.setFont("Helvetica", 10)
        c.drawRightString(right, y, f"Week: {monday.isoformat()} to {sunday.isoformat()}")
        y -= 16
        if selected_agent_code:
            c.drawRightString(right, y, f"Agent Filter: {selected_agent_code}")
        else:
            c.drawRightString(right, y, "Agent Filter: ALL")
        return y - 18
    ...
    def need_space(current_y, needed=90):
        if current_y < bottom + needed:
            return new_page()
        return current_y

    def draw_day_table(title, counts, current_y):
        current_y = need_space(current_y, 65)
        ...
        for d in days:
            c.rect(x, current_y - 12, col_w, 16)
            c.drawCentredString(x + col_w / 2, current_y - 1, d)
            x += col_w
        ...
        for n in counts:
            c.rect(x, current_y - 12, col_w, 16)
            c.drawCentredString(x + col_w / 2, current_y - 1, str(n))
            x += col_w

        return current_y - 22

    def draw_list(title, items, current_y, kind):
        current_y = need_space(current_y, 60)
        c.setFont("Helvetica-Bold", 10)
        c.drawString(left, current_y, title)
        current_y -= 14
        ...
        for label, x, w in cols:
            c.rect(x, current_y - 12, w, 16)
            c.drawString(x + 3, current_y - 1, label)
        current_y -= 16

        if not items:
            c.setFont("Helvetica", 8)
            c.rect(left, current_y - 12, 540, 16)
            c.drawString(left + 3, current_y - 1, f"No {kind} for this week")
            return current_y - 20

        c.setFont("Helvetica", 8)
        for item in items:
            current_y = need_space(current_y, 40)
            created = _pick(item, "created_at", "registered_at", "signup_date")
            di = _day_idx(created)
            day_name = days[di] if di is not None else ""

            vals = [
                (_row_name(item)[:34], left, 190),
                (_row_phone(item)[:16], left + 195, 95),
                (_row_town(item)[:16], left + 295, 95),
                (_row_code(item)[:16], left + 395, 95),
                (day_name, left + 495, 45),
            ]
            for val, x, w in vals:
                c.rect(x, current_y - 12, w, 16)
                c.drawString(x + 3, current_y - 1, str(val))
            current_y -= 16
    ...
    y = header()
    c.setFont("Helvetica", 9)
    c.drawString(left, y, f"Generated from Supabase | Agent groups with activity: {len(rows)}")
    y -= 18

    if not rows:
        c.setFont("Helvetica-Bold", 12)
        c.drawString(left, y, "No driver/client rows found for the selected week or agent.")
        c.save()
    ...
    for idx, row in enumerate(rows, start=1):
        y = need_space(y, 170)
        c.setFont("Helvetica-Bold", 12)
        c.drawString(left, y, f"{idx}. Agent: {row['agent_name']}")
        c.setFont("Helvetica", 10)
        c.drawString(left + 260, y, f"Code: {row['code'] or '-'}")
        if row["phone"]:
            c.drawString(left + 430, y, f"Phone: {row['phone']}")
        y -= 18

        c.setFont("Helvetica", 9)
        c.drawString(left, y, f"Weekly totals -> Drivers: {len(row['drivers'])} | Clients: {len(row['clients'])}")
        y -= 18

        y = draw_day_table("Driver registrations by day", row["driver_days"], y)
        y = draw_day_table("Client registrations by day", row["client_days"], y)
        y = draw_list("Drivers", row["drivers"], y, "drivers")
        y = draw_list("Clients", row["clients"], y, "clients")
