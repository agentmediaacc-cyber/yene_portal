...
def api_admin_weekly_agent_report_pdf():
    ...
    def _money(v):
        return f"N${float(v or 0):,.2f}"

    def _agent_name(a):
        return _clean(_pick(a, "full_name", "name", "agent_name"))

    def _agent_code(a):
        return _clean(_pick(a, "referral_code", "agent_code", "code", "my_referral_code", "referred_by_code"))

    def _agent_phone(a):
        return _clean(_pick(a, "phone", "phone_number", "mobile"))

    def _row_agent_name(r):
        return _clean(_pick(r, "recruiter", "agent_name", "referred_by_name", "agent", "registered_by"))

    def _row_agent_code(r):
        return _clean(_pick(r, "referred_by_code", "referral_code", "agent_code", "code"))

    def _looks_like_uuid(v):
        s = str(v or "").strip().lower()
        return len(s) == 36 and s.count("-") == 4

    def _safe_agent_display_name(name, code):
        n = _clean(name)
        c = _clean(code)
        if n and not _looks_like_uuid(n):
            return n
        if c and not _looks_like_uuid(c):
            return c
        return "Unknown Agent"

    def _row_name(r):
        return _clean(_pick(r, "full_name", "name", "driver_name", "client_name"))

    def _row_phone(r):
        return _clean(_pick(r, "phone", "phone_number", "mobile"))

    def _row_status(r):
        return _clean(_pick(r, "status", "approval_status"))
    ...
    def remember_agent(row):
        name = _agent_name(row)
        code = _agent_code(row)
        phone = _agent_phone(row)
        if not name and not code:
            return

        key = _norm(code) if code else _norm(name)
        agents[key] = {
            "name": _safe_agent_display_name(name, code),
            "code": code,
            "phone": phone,
            "src": row,
        }
        ...
        if name:
            name_to_key[_norm(name)] = key
        if code:
            code_to_key[_norm(code)] = key
    ...
    def ensure_bucket(agent_name="", agent_code="", phone=""):
        key = _norm(agent_code) or _norm(agent_name)
        if not key:
            key = "unassigned"

        display_name = _safe_agent_display_name(agent_name, agent_code)

        if key not in report:
            report[key] = {
                "agent_name": display_name,
                "agent_code": _clean(agent_code),
                "phone": _clean(phone),
                "clients": [],
                "drivers": [],
                "days": [{"clients": [], "drivers": []} for _ in range(7)],
            }
        else:
            if display_name != "Unknown Agent" and (not report[key]["agent_name"] or report[key]["agent_name"] == "Unknown Agent" or _looks_like_uuid(report[key]["agent_name"])):
                report[key]["agent_name"] = display_name
            if agent_code and not report[key]["agent_code"]:
                report[key]["agent_code"] = _clean(agent_code)
            if phone and not report[key]["phone"]:
                report[key]["phone"] = _clean(phone)
        return report[key]

    def bucket_for_row(r):
        row_code = _row_agent_code(r)
        row_name = _row_agent_name(r)

        code_key = _norm(row_code)
        name_key = _norm(row_name)

        if code_key and code_key in report:
            return report[code_key]
        if name_key and name_key in report:
            return report[name_key]

        mapped_code_key = code_to_key.get(code_key)
        if mapped_code_key and mapped_code_key in agents:
            a = agents[mapped_code_key]
            return ensure_bucket(a["name"], a["code"], a["phone"])

        mapped_name_key = name_to_key.get(name_key)
        if mapped_name_key and mapped_name_key in agents:
            a = agents[mapped_name_key]
            return ensure_bucket(a["name"], a["code"], a["phone"])

        # if recruiter name exists, use that as the real bucket before falling back to code
        if row_name:
            return ensure_bucket(row_name, row_code, "")

        return ensure_bucket("", row_code, "")

    for a in agents.values():
        ensure_bucket(a["name"], a["code"], a["phone"])

    # also seed buckets from recruiter names seen in weekly activity
    for r in weekly_clients + weekly_drivers:
        rn = _row_agent_name(r)
        rc = _row_agent_code(r)
        if rn:
            ensure_bucket(rn, rc, "")
    ...
    for r in weekly_drivers:
        ...
        di = _day_idx(_row_created(r))
        if di is not None:
            b["days"][di]["drivers"].append(r)

    rows = [v for v in report.values() if v["clients"] or v["drivers"]]

    # merge duplicate buckets by real display name first, then code
    merged = {}
    for r in rows:
        merge_key = _norm(r.get("agent_name"))
        if not merge_key or merge_key == "unknown agent":
            merge_key = _norm(r.get("agent_code"))
        if not merge_key:
            merge_key = f"unknown::{len(merged)}"

        if merge_key not in merged:
            merged[merge_key] = r
        else:
            merged[merge_key]["clients"].extend(r.get("clients", []))
            merged[merge_key]["drivers"].extend(r.get("drivers", []))
            for i in range(7):
                merged[merge_key]["days"][i]["clients"].extend(r["days"][i]["clients"])
                merged[merge_key]["days"][i]["drivers"].extend(r["days"][i]["drivers"])
            if _looks_like_uuid(merged[merge_key].get("agent_name")) and not _looks_like_uuid(r.get("agent_name")):
                merged[merge_key]["agent_name"] = r.get("agent_name")
            if not merged[merge_key].get("agent_code") and r.get("agent_code"):
                merged[merge_key]["agent_code"] = r.get("agent_code")
            if not merged[merge_key].get("phone") and r.get("phone"):
                merged[merge_key]["phone"] = r.get("phone")
    ...
    rows.sort(key=lambda r: (_norm(r["agent_name"]), _norm(r["agent_code"])))

    def daily_bonus(client_count, driver_count):
        client_part = admin_client_bonus if (client_count > 0 and client_count >= admin_client_bonus_min) else 0.0
        driver_part = admin_driver_bonus if (driver_count > 0 and driver_count >= admin_driver_bonus_min) else 0.0
        return client_part + driver_part
    ...
    def header():
        ...
        c.setFont("Helvetica-Bold", 18)
        c.drawString(LEFT, H - 30, "YENE WEEKLY AGENT PAYMENT REPORT")

        c.setFont("Helvetica", 9)
        c.drawRightString(RIGHT, H - 28, f"Week: {monday.isoformat()} to {sunday.isoformat()}")
        c.drawRightString(RIGHT, H - 42, f"Agent Filter: {selected_agent_code or 'ALL'}")

        c.setFont("Helvetica-Bold", 9)
        c.drawString(LEFT, H - 50,
                     f"Client Pay: {_money(admin_client_payment)}   Driver Pay: {_money(admin_driver_payment)}   Client Bonus: {_money(admin_client_bonus)} if >= {admin_client_bonus_min}/day   Driver Bonus: {_money(admin_driver_bonus)} if >= {admin_driver_bonus_min}/day")
        return H - 86
    ...
    if not rows:
        ...
        c.drawString(LEFT, y, "No registrations found for the selected week / agent.")
        c.save()
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        base_pay = (total_clients * admin_client_payment) + (total_drivers * admin_driver_payment)
        bonus_total = 0.0
        for i in range(7):
            bonus_total += daily_bonus(len(row["days"][i]["clients"]), len(row["days"][i]["drivers"]))
        total_pay = base_pay + bonus_total

        grand_clients += total_clients
        grand_drivers += total_drivers
        grand_base += base_pay
        grand_bonus += bonus_total
        grand_total += total_pay

        c.setFillColor(BLUE)
        c.roundRect(LEFT, y - 28, RIGHT - LEFT, 24, 8, fill=1, stroke=0)
        c.setFillColor(colors.white)
        c.setFont("Helvetica-Bold", 11)
        c.drawString(LEFT + 8, y - 13, f"{idx}. Agent: {row['agent_name'] or 'Unknown Agent'}")
        c.setFont("Helvetica", 9)
        c.drawRightString(RIGHT - 8, y - 13, f"Code: {row['agent_code'] or '-'}")
        y -= 36
        ...
        c.roundRect(LEFT, y - 44, RIGHT - LEFT, 40, 8, fill=1, stroke=0)
        c.setFillColor(TEXT)
        c.setFont("Helvetica-Bold", 9)
        c.drawString(LEFT + 8, y - 14, f"Clients: {total_clients}")
        c.drawString(LEFT + 110, y - 14, f"Drivers: {total_drivers}")
        c.drawString(LEFT + 220, y - 14, f"Base Pay: {_money(base_pay)}")
        c.drawString(LEFT + 360, y - 14, f"Bonus: {_money(bonus_total)}")
        c.setFillColor(GREEN)
        c.drawString(LEFT + 480, y - 14, f"Total Pay: {_money(total_pay)}")
        y -= 54
        ...
        c.setFillColor(colors.white)
        c.setFont("Helvetica-Bold", 9)
        c.drawString(LEFT + 6, y - 4, "DAILY REGISTRATION SUMMARY")
        y -= 24

        c.setFillColor(TEXT)
        c.setFont("Helvetica-Bold", 8)
        day_x = [LEFT, LEFT+60, LEFT+120, LEFT+180, LEFT+240, LEFT+300, LEFT+360]
        for i, d in enumerate(DAYS):
            c.rect(day_x[i], y - 13, 52, 16, fill=0, stroke=1)
            c.drawCentredString(day_x[i] + 26, y - 2, d)
        y -= 16

        c.setFont("Helvetica", 8)
        for i in range(7):
            cc = len(row["days"][i]["clients"])
            dd = len(row["days"][i]["drivers"])
            txt = f"C:{cc} D:{dd}"
            c.rect(day_x[i], y - 13, 52, 16, fill=0, stroke=1)
            c.drawCentredString(day_x[i] + 26, y - 2, txt)
        y -= 24

        for i in range(7):
            y = need_space(y, 90)
            d_clients = row["days"][i]["clients"]
            d_drivers = row["days"][i]["drivers"]
            d_bonus = daily_bonus(len(d_clients), len(d_drivers))

            c.setFillColor(BLUE2)
            c.rect(LEFT, y - 15, RIGHT - LEFT, 17, fill=1, stroke=0)
            c.setFillColor(colors.white)
            c.setFont("Helvetica-Bold", 8)
            c.drawString(LEFT + 6, y - 4, f"{DAYS[i]} | { (monday + dt.timedelta(days=i)).isoformat() } | Clients: {len(d_clients)} | Drivers: {len(d_drivers)} | Bonus: {_money(d_bonus)}")
            y -= 22

            c.setFillColor(TEXT)
            c.setFont("Helvetica-Bold", 8)
            c.drawString(LEFT, y, "Clients")
            c.drawString(LEFT + 280, y, "Drivers")
            y -= 12

            max_len = max(len(d_clients), len(d_drivers), 1)
            c.setFont("Helvetica", 7)

            for n in range(max_len):
                y = need_space(y, 24)

                c.rect(LEFT, y - 11, 250, 14, fill=0, stroke=1)
                c.rect(LEFT + 280, y - 11, 250, 14, fill=0, stroke=1)

                if n < len(d_clients):
                    cl = d_clients[n]
                    txt = f"{_row_name(cl)} | {_row_agent_code(cl) or '-'} | {_row_phone(cl) or '-'}"
                    c.drawString(LEFT + 3, y - 2, txt[:54])

                if n < len(d_drivers):
                    dr = d_drivers[n]
                    txt = f"{_row_name(dr)} | {_row_agent_code(dr) or '-'} | {_row_phone(dr) or '-'}"
                    c.drawString(LEFT + 283, y - 2, txt[:54])
        ...
        y = need_space(y, 70)
        c.setFillColor(LIGHT)
        c.roundRect(LEFT, y - 36, RIGHT - LEFT, 34, 8, fill=1, stroke=0)
        c.setFillColor(TEXT)
        c.setFont("Helvetica-Bold", 9)
        c.drawString(LEFT + 8, y - 14, f"PAYMENT BREAKDOWN -> Clients: {total_clients} x {_money(admin_client_payment)} + Drivers: {total_drivers} x {_money(admin_driver_payment)} + Bonus: {_money(bonus_total)}")
        c.setFillColor(GREEN)
        c.drawString(LEFT + 8, y - 28, f"TOTAL TO PAY {row['agent_name'] or 'AGENT'}: {_money(total_pay)}")
        y -= 48
    ...
    y = need_space(y, 90)
    c.setFillColor(BLUE)
    c.roundRect(LEFT, y - 48, RIGHT - LEFT, 44, 10, fill=1, stroke=0)
    c.setFillColor(colors.white)
    c.setFont("Helvetica-Bold", 12)
    c.drawString(LEFT + 10, y - 16, "GRAND TOTAL FOR ALL AGENTS")
    c.setFont("Helvetica", 10)
    c.drawString(LEFT + 10, y - 32,
                 f"Clients: {grand_clients} | Drivers: {grand_drivers} | Base Pay: {_money(grand_base)} | Bonus: {_money(grand_bonus)} | TOTAL PAYOUT: {_money(grand_total)}")
