  ...
  <div class="wrap">
    ...
    <div class="card">
      ...
      <div class="grid">
        ...
        <div>
          <label for="client_bonus">Client bonus per registration</label>
          <input type="number" id="client_bonus" step="0.01" min="0" value="15">
        </div>

        <div>
          <label for="driver_bonus">Driver bonus per registration</label>
...
def api_admin_weekly_agent_report_pdf():
    ...
    def _daily_bonus_for_agent(agent_row, client_count, driver_count):
        client_bonus = _client_bonus_for_agent(agent_row) * client_count
        driver_bonus = _driver_bonus_for_agent(agent_row) * driver_count
        return client_bonus + driver_bonus
    ...
    def draw_pay_table(row, y):
        ...
        rows_pay = [
            ...
            ("Client Bonus Rate", "-", client_bonus_rate, len(row["clients"]) * client_bonus_rate),
            ("Driver Bonus Rate", "-", driver_bonus_rate, len(row["drivers"]) * driver_bonus_rate),
    ...
    def draw_day_section(day_name, day_date, drivers, clients, y):
        ...
        c.drawString(LEFT + 8, y - 4, f"{day_name}  |  {day_date.isoformat()}  |  Clients: {len(clients)}  |  Drivers: {len(drivers)}  |  Bonus: {money(_daily_bonus_for_agent(agent_for_bonus, len(clients), len(drivers)))}")
        y -= 24
