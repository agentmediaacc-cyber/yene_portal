...
def api_admin_weekly_agent_report_pdf():
    ...
    def _agent_name(a):
        return _clean(_pick(a, "full_name", "name", "agent_name", "username"))

    def _agent_code(a):
        return _clean(_pick(a, "referral_code", "agent_code", "code", "my_referral_code"))

    def _agent_phone(a):
        return _clean(_pick(a, "phone", "phone_number", "mobile"))

    def _agent_email(a):
        return _clean(_pick(a, "email"))

    def _row_agent_names(r):
        vals = [
            _pick(r, "recruiter"),
            _pick(r, "agent_name"),
            _pick(r, "referred_by_name"),
            _pick(r, "registered_by"),
            _pick(r, "agent"),
            _pick(r, "referred_by"),
        ]
        return [_clean(v) for v in vals if _clean(v)]

    def _row_agent_codes(r):
        vals = [
            _pick(r, "referred_by_code"),
            _pick(r, "referral_code"),
            _pick(r, "agent_code"),
            _pick(r, "code"),
        ]
        return [_clean(v) for v in vals if _clean(v)]
    ...
    def _row_town(r):
        return _clean(_pick(r, "town", "city", "location", "region"))

    def _daily_bonus(client_count, driver_count):
    ...
    # Build real agents list
    raw_agents = _safe_rows("agent_profiles") + _safe_rows("agents")
    seen = set()
    agents = []
    ...
    for a in raw_agents:
        name = _agent_name(a)
        code = _agent_code(a)
        phone = _agent_phone(a)
        email = _agent_email(a)
        key = _norm(code) or _norm(name) or _norm(email) or _norm(phone)
        if not key:
            continue
        if key in seen:
            continue
    ...
    def _matches_agent(row, agent):
        agent_names = {
            _norm(agent.get("name")),
            _norm(_pick(agent.get("src", {}), "username")),
        }
        agent_codes = {
            _norm(agent.get("code")),
        }

        row_names = {_norm(v) for v in _row_agent_names(row)}
        row_codes = {_norm(v) for v in _row_agent_codes(row)}
        ...
        if agent_codes and row_codes and (agent_codes & row_codes):
            return True

        if agent_names and row_names and (agent_names & row_names):
            return True
    ...
    for agent in agents:
        ...
        if bucket["clients"] or bucket["drivers"]:
            rows.append(bucket)
    ...
    for row in rows:
        for r in row["clients"]:
            rid = _clean(_pick(r, "id"))
            if rid:
                matched_client_ids.add(rid)
        for r in row["drivers"]:
            rid = _clean(_pick(r, "id"))
            if rid:
                matched_driver_ids.add(rid)
    ...
    for r in weekly_clients:
        rid = _clean(_pick(r, "id"))
        if not rid or rid not in matched_client_ids:
            unmatched["clients"].append(r)
            di = _day_idx(_row_created(r))
            if di is not None:
                unmatched["days"][di]["clients"].append(r)

    for r in weekly_drivers:
        rid = _clean(_pick(r, "id"))
        if not rid or rid not in matched_driver_ids:
            unmatched["drivers"].append(r)
            di = _day_idx(_row_created(r))
            if di is not None:
                unmatched["days"][di]["drivers"].append(r)

    if unmatched["clients"] or unmatched["drivers"]:
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        bonus_total = sum(_daily_bonus(len(row["days"][i]["clients"]), len(row["days"][i]["drivers"])) for i in range(7))
        ...
        for i in range(7):
            ...
            d_bonus = _daily_bonus(len(d_clients), len(d_drivers))
            ...
            for n in range(max_len):
                ...
                if n < len(d_clients):
                    cl = d_clients[n]
                    txt = f"{_row_name(cl)} | {_row_phone(cl) or '-'} | {_row_status(cl) or '-'}"
                    c.drawString(LEFT + 3, y - 2, txt[:56])

                if n < len(d_drivers):
                    dr = d_drivers[n]
                    txt = f"{_row_name(dr)} | {_row_phone(dr) or '-'} | {_row_status(dr) or '-'}"
                    c.drawString(LEFT + 283, y - 2, txt[:56])
