...
def api_admin_weekly_agent_report_pdf():
    ...
    def _find_rate(possible_keys, fallback=0.0):
        for row in payment_rules:
            text = " ".join([str(v) for v in row.values()]).lower()
            for key in possible_keys:
                if key in text:
                    for amount_key in ["amount", "value", "rate", "price", "pay", "payment_amount"]:
                        if amount_key in row and row.get(amount_key) not in (None, ""):
                            return _num(row.get(amount_key), fallback)
        return float(fallback)

    client_rate = _find_rate(["client", "customer"], 0.0)
    driver_rate = _find_rate(["driver"], 0.0)
    ...
    def _bonus_rules():
        ...
        for row in payment_rules:
            ...
            threshold = None
            amount = None
            for k in ["min_clients", "minimum_clients", "threshold", "target", "from_count", "daily_clients", "client_count", "min_count"]:
                if row.get(k) not in (None, ""):
                    threshold = int(float(row.get(k)))
                    break
            for k in ["bonus_amount", "amount", "value", "rate"]:
                if row.get(k) not in (None, ""):
                    amount = _num(row.get(k), 0.0)
                    break
            if threshold is not None and amount is not None:
                rules.append((threshold, amount))
    ...
    BONUS_RULES = _bonus_rules()

    def _daily_bonus(client_count):
        bonus = 0.0
        for threshold, amount in BONUS_RULES:
    ...
    def draw_summary_cards(row, y):
        ...
        daily_bonus_total = sum(_daily_bonus(len(row["days"][i]["clients"])) for i in range(7))
    ...
    def draw_pay_table(row, y):
        ...
        rows_pay = [
            ...
            ("Daily Client Bonus", "-", "-", sum(_daily_bonus(len(row["days"][i]["clients"])) for i in range(7))),
        ]
        ...
        total = (len(row["drivers"]) * driver_rate) + (len(row["clients"]) * client_rate) + sum(_daily_bonus(len(row["days"][i]["clients"])) for i in range(7))
        c.setFillColor(BRAND)
    ...
    def draw_day_section(day_name, day_date, drivers, clients, y):
        ...
        c.setFillColor(colors.white)
        c.setFont("Helvetica-Bold", 10)
        c.drawString(LEFT + 8, y - 4, f"{day_name}  |  {day_date.isoformat()}  |  Clients: {len(clients)}  |  Drivers: {len(drivers)}  |  Bonus: {money(_daily_bonus(len(clients)))}")
        y -= 24
