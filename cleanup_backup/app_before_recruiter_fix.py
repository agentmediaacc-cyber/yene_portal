...
def api_admin_weekly_agent_report_pdf():
    ...
    # Build lookup from real agent tables so names and codes can enrich recruiter rows
    name_lookup = {}
    code_lookup = {}

    for a in (_safe_rows("agent_profiles") + _safe_rows("agents")):
        nm = _clean(_pick(a, "full_name", "name", "agent_name", "username"))
        cd = _clean(_pick(a, "referral_code", "agent_code", "code", "my_referral_code"))
        ph = _clean(_pick(a, "phone", "phone_number", "mobile"))
        if nm:
            name_lookup[_norm(nm)] = {"name": nm, "code": cd, "phone": ph}
        if cd:
            code_lookup[_norm(cd)] = {"name": nm or cd, "code": cd, "phone": ph}
    ...
    def ensure_bucket(agent_name="", agent_code="", phone=""):
        name = _clean(agent_name)
        code = _clean(agent_code)
        key = _norm(name) or _norm(code) or "unassigned"

        if key not in report:
            report[key] = {
                "agent_name": name or "Unassigned / Unknown Agent",
                "agent_code": code,
                "phone": _clean(phone),
                "clients": [],
                "drivers": [],
            ...
            if name and report[key]["agent_name"] == "Unassigned / Unknown Agent":
    ...
    def bucket_for_row(r):
        recruiter_name = _row_agent_name(r)
        recruiter_code = _row_agent_code(r)

        if recruiter_name:
            lk = name_lookup.get(_norm(recruiter_name))
            if lk:
                return ensure_bucket(lk["name"], lk["code"], lk["phone"])
            return ensure_bucket(recruiter_name, recruiter_code, "")

        if recruiter_code:
            lk = code_lookup.get(_norm(recruiter_code))
            if lk:
                return ensure_bucket(lk["name"], lk["code"], lk["phone"])
            return ensure_bucket("", recruiter_code, "")
    ...
    def header():
        ...
        c.drawString(LEFT, H - 28, "YENE WEEKLY AGENT PAYMENT REPORT")
        ...
        c.drawString(LEFT, H - 48, f"Client Pay: {_money(admin_client_payment)}   Driver Pay: {_money(admin_driver_payment)}   Client Bonus: {_money(admin_client_bonus)} if >= {admin_client_bonus_min}/day   Driver Bonus: {_money(admin_driver_bonus)} if >= {admin_driver_bonus_min}/day")
        return H - 86
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        c.setFillColor(colors.white)
        c.setFont("Helvetica-Bold", 11)
        c.drawString(LEFT + 8, y - 13, f"{idx}. Agent: {row['agent_name'] or 'Unassigned / Unknown Agent'}")
        c.setFont("Helvetica", 9)
        ...
        y = need_space(y, 60)
        c.setFillColor(LIGHT)
        c.roundRect(LEFT, y - 34, RIGHT - LEFT, 32, 8, fill=1, stroke=0)
        c.setFillColor(TEXT)
        c.setFont("Helvetica-Bold", 9)
        c.drawString(LEFT + 8, y - 12, f"PAYMENT BREAKDOWN -> Clients: {total_clients} x {_money(admin_client_payment)} + Drivers: {total_drivers} x {_money(admin_driver_payment)} + Bonus: {_money(bonus_total)}")
        c.setFillColor(GREEN)
        c.drawString(LEFT + 8, y - 25, f"TOTAL TO PAY {row['agent_name'] or 'AGENT'}: {_money(total_pay)}")
        y -= 44
    ...
    y = need_space(y, 80)
    c.setFillColor(BLUE)
    c.roundRect(LEFT, y - 46, RIGHT - LEFT, 42, 10, fill=1, stroke=0)
    c.setFillColor(colors.white)
    c.setFont("Helvetica-Bold", 12)
    c.drawString(LEFT + 10, y - 16, "GRAND TOTAL FOR ALL AGENTS")
    c.setFont("Helvetica", 10)
    c.drawString(LEFT + 10, y - 31, f"Clients: {grand_clients} | Drivers: {grand_drivers} | Base Pay: {_money(grand_base)} | Bonus: {_money(grand_bonus)} | TOTAL PAYOUT: {_money(grand_total)}")
