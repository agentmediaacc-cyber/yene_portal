...
@app.route("/api/admin/namibia_regions")
def api_admin_regions():
    if not _require_admin():
        return jsonify({"error": "Unauthorized"}), 403

    regions = [
        {"region": "Khomas", "town": "Windhoek"},
        {"region": "Erongo", "town": "Swakopmund"},
        {"region": "Erongo", "town": "Walvis Bay"},
        {"region": "Kavango East", "town": "Rundu"},
    ]

    return jsonify({"regions": regions})


@app.route("/api/admin/finance_summary_by_region")
def api_admin_finance_summary_by_region():
    if not _require_admin():
        return jsonify({"error": "Unauthorized"}), 403

    try:
        agents = sb_admin.table("profiles").select("*").execute().data or []
        return jsonify({"summary": []})  # you can improve later
    except Exception as e:
        return jsonify({"error": str(e)}), 500


@app.route("/api/admin/agent_payment_due")
def api_admin_agent_payment_due():
    if not _require_admin():
        return jsonify({"error": "Unauthorized"}), 403
...
def _parse_week_start_arg():
    ...
    if raw:
        try:
            monday = dt.date.fromisoformat(raw)
            monday = monday - dt.timedelta(days=monday.weekday())
            return monday
        except Exception:
            pass
...
def api_admin_weekly_agent_report_pdf():
    ...
    monday = _parse_week_start_arg()
    sunday = monday + dt.timedelta(days=6)

    # Pull agents from both tables and merge by email
    agents_map = {}

    try:
        rows = sb_admin.table("agent_profiles").select("*").limit(5000).execute().data or []
        for a in rows:
    ...
    try:
        rows = sb_admin.table("agents").select("*").limit(5000).execute().data or []
        for a in rows:
            email = (a.get("email") or "").strip().lower()
            if email and email not in agents_map:
                agents_map[email] = a
    except Exception:
        pass
    ...
    end_iso = f"{sunday.isoformat()}T23:59:59"
    ...
    # Build lookup by agent id
    agent_by_id = {}
    for a in agents:
        if a.get("id"):
            agent_by_id[str(a.get("id"))] = a

    # Aggregate per agent + day
    summary = {}
    details = defaultdict(list)

    def ensure_agent(agent_id, fallback_name="Unknown Agent", fallback_email=""):
        if agent_id not in summary:
            a = agent_by_id.get(str(agent_id), {}) if agent_id else {}
            summary[agent_id] = {
                "agent_id": agent_id,
                "agent_name": a.get("full_name") or a.get("username") or fallback_name,
                "email": a.get("email") or fallback_email,
                "days": {
                    "Mon": {"drivers": 0, "clients": 0},
                    "Tue": {"drivers": 0, "clients": 0},
                    "Wed": {"drivers": 0, "clients": 0},
                    "Thu": {"drivers": 0, "clients": 0},
                    "Fri": {"drivers": 0, "clients": 0},
                    "Sat": {"drivers": 0, "clients": 0},
                    "Sun": {"drivers": 0, "clients": 0},
                },
                "drivers_total": 0,
                "clients_total": 0,
            }
    ...
    for d in drivers:
        ...
        if not created:
            continue
        agent_id = str(d.get("recruiter_agent_id") or "")
        recruiter_name = d.get("recruiter_name") or "Unknown Agent"
        agent_rec = ensure_agent(agent_id, recruiter_name)
        day = _day_key(created.date())
        agent_rec["days"][day]["drivers"] += 1
        agent_rec["drivers_total"] += 1

        details[agent_id].append({
            "created_at": created,
            "type": "Driver",
            "name": d.get("full_name") or "",
            "phone": d.get("phone") or d.get("phone_number") or "",
            "town": d.get("town") or "",
            "code": d.get("license_number") or d.get("driver_code") or d.get("external_code") or "",
        })

    for c in clients:
        created = _safe_dt(c.get("created_at"))
        if not created:
            continue
        agent_id = str(c.get("recruiter_agent_id") or "")
        recruiter_name = c.get("recruiter_name") or "Unknown Agent"
        agent_rec = ensure_agent(agent_id, recruiter_name)
        day = _day_key(created.date())
        agent_rec["days"][day]["clients"] += 1
        agent_rec["clients_total"] += 1
        ...
        details[agent_id].append({
            ...
            "town": c.get("town") or "",
            "code": c.get("yene_code") or c.get("client_code") or c.get("external_code") or "",
        })

    rows = list(summary.values())
    rows.sort(key=lambda x: (x["agent_name"] or "").lower())
    ...
    def new_page(title=None):
        ...
        c.drawString(40, y, "YENE Weekly Agent Registration Report")
        y -= 18
        c.setFont("Helvetica", 10)
        c.drawString(40, y, f"Week: {monday.isoformat()} to {sunday.isoformat()}")
        if title:
            y -= 18
    ...
    c.drawString(155, y, "Mon")
    c.drawString(200, y, "Tue")
    c.drawString(245, y, "Wed")
    c.drawString(290, y, "Thu")
    c.drawString(335, y, "Fri")
    c.drawString(380, y, "Sat")
    c.drawString(425, y, "Sun")
    c.drawString(480, y, "Total")
    y -= 16

    c.setFont("Helvetica", 8)
    if not rows:
        c.drawString(40, y, "No registrations found for this week.")
        y -= 18
    ...
    for r in rows:
        ...
        c.drawString(40, y, (r["agent_name"] or "Unknown")[:22])

        for xpos, day in zip(
            [155, 200, 245, 290, 335, 380, 425],
            ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
        ):
            val = f"{r['days'][day]['drivers']}/{r['days'][day]['clients']}"
            c.drawString(xpos, y, val)

        total_txt = f"{r['drivers_total']}/{r['clients_total']}"
        c.drawString(480, y, total_txt)
        y -= 14

    # Detail pages per agent
    for r in rows:
        y = new_page(f"Agent: {r['agent_name']}")

        c.setFont("Helvetica", 10)
        c.drawString(40, y, f"Email: {r.get('email') or '-'}")
        y -= 14
        c.drawString(40, y, f"Drivers: {r['drivers_total']}    Clients: {r['clients_total']}")
        y -= 18
        ...
        c.drawString(390, y, "Town")
        c.drawString(470, y, "Code")
        y -= 14

        c.setFont("Helvetica", 8)
        agent_details = sorted(details.get(r["agent_id"], []), key=lambda x: x["created_at"])

        if not agent_details:
            c.drawString(40, y, "No detailed registrations for this agent in the selected week.")
            y -= 14

        for item in agent_details:
            if y < 55:
                y = new_page(f"Agent: {r['agent_name']} (continued)")
                c.setFont("Helvetica-Bold", 9)
                c.drawString(40, y, "Time")
                c.drawString(120, y, "Type")
                c.drawString(170, y, "Name")
            ...
            c.drawString(40, y, item["created_at"].strftime("%a %H:%M"))
            c.drawString(120, y, item["type"][:8])
            c.drawString(170, y, str(item["name"])[:24])
            c.drawString(300, y, str(item["phone"])[:14])
            c.drawString(390, y, str(item["town"])[:12])
            c.drawString(470, y, str(item["code"])[:16])
            y -= 12
...
def _parse_week_start_arg():
    ...
    if raw:
        try:
            monday = dt.date.fromisoformat(raw)
            return monday - dt.timedelta(days=monday.weekday())
        except Exception:
            pass
    today = dt.date.today()
...
def api_admin_weekly_agent_report_pdf():
    ...
    agents = list(agents_map.values())
    agent_by_id = {str(a.get("id")): a for a in agents if a.get("id")}
    ...
    for d in drivers:
        ...
        if not created:
            continue
        agent_id = str(d.get("recruiter_agent_id") or "")
        agent_rec = ensure_agent(agent_id, d.get("recruiter_name") or "Unknown Agent")
        day = _day_key(created.date())
        agent_rec["days"][day]["drivers"] += 1
        agent_rec["drivers_total"] += 1
        details[agent_id].append({
    ...
    for c_row in clients:
        created = _safe_dt(c_row.get("created_at"))
        if not created:
            continue
        agent_id = str(c_row.get("recruiter_agent_id") or "")
        agent_rec = ensure_agent(agent_id, c_row.get("recruiter_name") or "Unknown Agent")
        day = _day_key(created.date())
        agent_rec["days"][day]["clients"] += 1
        agent_rec["clients_total"] += 1
        details[agent_id].append({
            "created_at": created,
            "type": "Client",
            "name": c_row.get("full_name") or "",
            "phone": c_row.get("phone") or c_row.get("phone_number") or "",
            "town": c_row.get("town") or "",
            "code": c_row.get("yene_code") or c_row.get("client_code") or c_row.get("external_code") or "",
        })
    ...
    for r in rows:
        ...
        c.drawString(40, y, (r["agent_name"] or "Unknown")[:22])
        for xpos, day in zip([155,200,245,290,335,380,425], ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"]):
            c.drawString(xpos, y, f"{r['days'][day]['drivers']}/{r['days'][day]['clients']}")
        c.drawString(480, y, f"{r['drivers_total']}/{r['clients_total']}")
        y -= 14

    for r in rows:
        c.showPage()
        y = page_header(f"Agent: {r['agent_name']}")
        c.setFont("Helvetica", 10)
        c.drawString(40, y, f"Email: {r.get('email') or '-'}")
        y -= 14
        c.drawString(40, y, f"Drivers: {r['drivers_total']}    Clients: {r['clients_total']}")
        ...
        agent_details = sorted(details.get(r["agent_id"], []), key=lambda x: x["created_at"])
        if not agent_details:
            c.drawString(40, y, "No detailed registrations for this agent in this week.")
            y -= 14

        for item in agent_details:
            if y < 55:
                c.showPage()
                y = page_header(f"Agent: {r['agent_name']} (continued)")
                c.setFont("Helvetica-Bold", 9)
                c.drawString(40, y, "Time")
                c.drawString(120, y, "Type")
                c.drawString(170, y, "Name")
