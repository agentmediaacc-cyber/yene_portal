...
from admin_agents_reject_fix_routes import \
    register_admin_agents_reject_fix_routes
from admin_approval_working_routes import \
    register_admin_approval_working_routes
...
def decorator(fn):
    ...
    def wrapped(*args, **kwargs):
        ...
        if (
            ...
            and not session.get("email")
            and not session.get("role")
        ):
            flash("Please log in.")
...
def api_admin_wallet_ledger():
    try:
        res = (
            supabase.table("agent_wallet_ledger")
            .select("*")
            .order("created_at", desc=True)
            .execute()
        )
...
def api_admin_assign_agent_region(agent_id):
    try:
        ...
        if not payload:
            return (
                jsonify({"success": False, "error": "No region or town provided"}),
                400,
            )
...
def api_admin_agent_wallet_fund(agent_id):
    try:
        ...
        agent_res = (
            supabase.table("agents").select("*").eq("id", agent_id).limit(1).execute()
        )
