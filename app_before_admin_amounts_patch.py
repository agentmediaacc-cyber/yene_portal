...
def api_admin_weekly_agent_report_pdf():
    ...
    def _rule_matches_agent(rule, agent_row):
        ...
        rule_region = _norm(rule.get("region"))
        rule_town = _norm(rule.get("town"))

        if rule_town and agent_town and rule_town == agent_town:
            return True
        if rule_region and agent_region and rule_region == agent_region:
            return True
        return False
    ...
    def _first_trip_bonus_for_agent(agent_row):
        return _num(_best_rule_for_agent(agent_row).get("first_trip_bonus"), 0.0)

    def _referral_pct_for_agent(agent_row):
        return _num(_best_rule_for_agent(agent_row).get("referral_pct"), 0.0)

    def _daily_bonus_for_agent(agent_row, client_count):
        bonus_per_day = _first_trip_bonus_for_agent(agent_row)
        if client_count <= 0:
            return 0.0
        return client_count * bonus_per_day
    ...
    def draw_summary_cards(row, y):
        ...
        daily_bonus_total = sum(_daily_bonus_for_agent(row["agent"], len(row["days"][i]["clients"])) for i in range(7))
    ...
    def draw_pay_table(row, y):
        ...
        driver_rate = _driver_rate_for_agent(row["agent"])
        client_rate = _client_rate_for_agent(row["agent"])
        total_bonus = sum(_daily_bonus_for_agent(row["agent"], len(row["days"][i]["clients"])) for i in range(7))
        ...
        rows_pay = [
            ...
            ("Daily Client Bonus", "-", "-", total_bonus),
        ]
    ...
    def draw_day_section(day_name, day_date, drivers, clients, y):
        ...
        c.drawString(LEFT + 8, y - 4, f"{day_name}  |  {day_date.isoformat()}  |  Clients: {len(clients)}  |  Drivers: {len(drivers)}  |  Bonus: {money(_daily_bonus_for_agent(agent_for_bonus, len(clients)))}")
        y -= 24
