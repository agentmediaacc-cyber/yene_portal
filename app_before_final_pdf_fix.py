...
def api_admin_agents():
    ...
    def _safe_table_rows(table_name):
        try:
    ...
    def upsert_agent(row, source_name, forced_name=None):
        name = _clean_name(forced_name or _pick(row, "full_name", "name", "agent_name"))
        email = _norm(_pick(row, "email"))
        code = _norm(_pick(row, "referral_code", "agent_code", "code", "my_referral_code", "referred_by_code"))
        key = email or code or _norm(name) or _norm(_pick(row, "id"))
        if not key:
            return
        ...
        if key not in merged:
            merged[key] = {
                ...
                "town": _pick(row, "town", "city", "location"),
                "region": _pick(row, "region"),
                "referral_code": _pick(row, "referral_code", "agent_code", "code", "my_referral_code", "referred_by_code"),
                "status": _pick(row, "status", "approval_status"),
            ...
            existing = merged[key]
            existing["id"] = existing["id"] or _pick(row, "id")
            existing["full_name"] = existing["full_name"] or name
            existing["email"] = existing["email"] or _pick(row, "email")
            existing["phone"] = existing["phone"] or _pick(row, "phone", "mobile", "phone_number")
            existing["town"] = existing["town"] or _pick(row, "town", "city", "location")
            existing["region"] = existing["region"] or _pick(row, "region")
            existing["referral_code"] = existing["referral_code"] or _pick(row, "referral_code", "agent_code", "code", "my_referral_code", "referred_by_code")
            existing["status"] = existing["status"] or _pick(row, "status", "approval_status")
            existing["created_at"] = existing["created_at"] or _pick(row, "created_at")

    for row in _safe_table_rows("agent_profiles"):
        upsert_agent(row, "agent_profiles")

    for row in _safe_table_rows("agents"):
        upsert_agent(row, "agents")

    clients_rows = _safe_table_rows("clients")
    drivers_rows = _safe_table_rows("drivers")

    for row in clients_rows:
        recruiter_name = _clean_name(_pick(row, "recruiter", "agent_name", "referred_by_name", "agent", "registered_by"))
        recruiter_code = _pick(row, "referred_by_code", "referral_code", "agent_code", "code")
        fake = {
            "full_name": recruiter_name,
            "referral_code": recruiter_code,
            "town": _pick(row, "town", "city", "location"),
            "region": _pick(row, "region"),
            "created_at": _pick(row, "created_at", "registered_at", "signup_date"),
        }
        if recruiter_name:
            upsert_agent(fake, "clients_recruiter", forced_name=recruiter_name)
            key = _norm(recruiter_code) or _norm(recruiter_name)
            if key in merged:
                merged[key]["clients_count"] = int(merged[key].get("clients_count") or 0) + 1
    ...
    for row in drivers_rows:
        ...
        if recruiter_name:
            upsert_agent(fake, "drivers_recruiter", forced_name=recruiter_name)
            key = _norm(recruiter_code) or _norm(recruiter_name)
            if key in merged:
                merged[key]["drivers_count"] = int(merged[key].get("drivers_count") or 0) + 1

    rows = list(merged.values())
    rows = [r for r in rows if _clean_name(r.get("full_name"))]
    rows.sort(key=lambda r: (str(r.get("full_name") or "").lower(), str(r.get("email") or "").lower()))
...
def admin_agent_profile_page_old(agent_id):
    if session.get("role") != "ADMIN":
        return redirect(url_for("admin_login"))
...
(function () {
  ...
  fetch("/api/admin/agents")
    .then(r => r.json())
    .then(data => {
      const agents = Array.isArray(data) ? data : (data.rows || data.agents || data.data || []);
      data = agents;
...
def api_admin_weekly_agent_report_pdf():
    ...
    def _row_agent_name(r):
        return str(_pick(r, "recruiter", "agent_name", "referred_by_name", "agent", "registered_by")).strip()
    ...
    def _remember_agent(agent_row, forced_name=None):
        code = _agent_code(agent_row)
        name = " ".join(str(forced_name or _agent_name(agent_row)).strip().split())
        if code:
            agents_map[_norm(code)] = agent_row
        if name:
            agents_name_map[_norm(name)] = agent_row
    ...
    def _ensure_report_bucket(agent_row=None, forced_name="", forced_code=""):
        code = str(forced_code or _agent_code(agent_row or {})).strip()
        name = " ".join(str(forced_name or _agent_name(agent_row or {})).strip().split())
        key = _norm(code) or _norm(name)
        if not key:
            key = "unassigned"
        ...
        if key not in report:
            report[key] = {
                ...
                "agent_code": code or "",
                "phone": _agent_phone(agent_row or {}),
                "drivers": [],
                "clients": [],
                "days": [
            ...
            if agent_row:
                report[key]["agent"] = report[key]["agent"] or agent_row
                report[key]["phone"] = report[key]["phone"] or _agent_phone(agent_row)
            if name and (not report[key]["agent_name"] or report[key]["agent_name"] == "Unknown Agent"):
                report[key]["agent_name"] = name
            if code and not report[key]["agent_code"]:
                report[key]["agent_code"] = code

        if code:
            report[key]["agent_code"] = report[key]["agent_code"] or code
        if name:
            report[key]["agent_name"] = report[key]["agent_name"] or name

        return report[key]

    for code_norm, agent in agents_map.items():
        _ensure_report_bucket(agent_row=agent)

    for name_norm, agent in agents_name_map.items():
        _ensure_report_bucket(agent_row=agent)

    for r in weekly_drivers:
        code_norm = _norm(_row_code(r))
        name_norm = _norm(" ".join(_row_agent_name(r).split()))
        bucket = None

        if code_norm and code_norm in report:
            bucket = report[code_norm]
        elif name_norm and name_norm in report:
            bucket = report[name_norm]
        else:
            agent_match = agents_map.get(code_norm) or agents_name_map.get(name_norm) or {}
            bucket = _ensure_report_bucket(
                agent_row=agent_match,
                forced_name=_row_agent_name(r),
                forced_code=_row_code(r),
            )

        if _row_agent_name(r) and (not bucket.get("agent_name") or bucket.get("agent_name") == "Unknown Agent"):
            bucket["agent_name"] = _row_agent_name(r)
        if _row_code(r) and not bucket.get("agent_code"):
            bucket["agent_code"] = _row_code(r)
    ...
    dedup = {}
    for r in rows:
        k = _norm(r.get("agent_code")) or _norm(" ".join(str(r.get("agent_name") or "").split()))
        if not k:
            k = f"unknown::{len(dedup)}"
        if k not in dedup:
            dedup[k] = r
        else:
            dedup[k]["drivers"].extend(r.get("drivers", []))
            dedup[k]["clients"].extend(r.get("clients", []))
            for i in range(7):
                dedup[k]["days"][i]["drivers"].extend(r["days"][i]["drivers"])
                dedup[k]["days"][i]["clients"].extend(r["days"][i]["clients"])
            if not dedup[k].get("agent_name") and r.get("agent_name"):
                dedup[k]["agent_name"] = r["agent_name"]
            if not dedup[k].get("agent_code") and r.get("agent_code"):
                dedup[k]["agent_code"] = r["agent_code"]
...
@app.after_request
def inject_agent_center_button(response):
    try:
        if request.path != "/dashboard/admin":
            return response
        ctype = (response.content_type or "").lower()
        ...
        body = response.get_data(as_text=True)
        if "/admin/agents-center" in body:
            return response

        inject = """
<div id="admin-agent-center-shortcut" style="position:fixed;left:18px;bottom:18px;z-index:99999;">
  <a href="/admin/agents-center"
...
@app.route("/api/admin/agents_full", methods=["GET"])
def api_admin_agents_full():
    from flask import jsonify, session
    ...
    def ensure_agent(base=None, forced_name=None, source=""):
        base = base or {}
        name = _clean(forced_name or _pick(base, "full_name", "name", "agent_name", "recruiter"))
        code = _pick(base, "referral_code", "agent_code", "code", "my_referral_code", "referred_by_code")
        email = _pick(base, "email")
        key = _norm(email) or _norm(code) or _norm(name) or _norm(_pick(base, "id"))
        if not key:
            return None
        ...
        if key not in merged:
            merged[key] = {
                ...
                "full_name": name,
                "email": email,
                "phone": _pick(base, "phone", "mobile", "phone_number"),
                "referral_code": code,
                "town": _pick(base, "town", "city", "location"),
                "region": _pick(base, "region"),
                "status": _pick(base, "status", "approval_status") or "pending_approval",
                "created_at": _pick(base, "created_at", "registered_at", "signup_date"),
                "clients_count": 0,
                "drivers_count": 0,
                "source_table": source or "unknown",
            }
        else:
            row = merged[key]
            row["id"] = row["id"] or _pick(base, "id")
            row["full_name"] = row["full_name"] or name
            row["email"] = row["email"] or email
            row["phone"] = row["phone"] or _pick(base, "phone", "mobile", "phone_number")
            row["referral_code"] = row["referral_code"] or code
            row["town"] = row["town"] or _pick(base, "town", "city", "location")
            row["region"] = row["region"] or _pick(base, "region")
            row["status"] = row["status"] or _pick(base, "status", "approval_status")
            row["created_at"] = row["created_at"] or _pick(base, "created_at", "registered_at", "signup_date")
        return merged[key]

    agent_profiles = _safe_rows("agent_profiles")
    agents = _safe_rows("agents")
    clients = _safe_rows("clients")
    drivers = _safe_rows("drivers")

    for a in agent_profiles:
        ensure_agent(a, source="agent_profiles")

    for a in agents:
        ensure_agent(a, source="agents")

    for r in clients:
        recruiter_name = _clean(_pick(r, "recruiter", "agent_name", "referred_by_name", "agent", "registered_by"))
        recruiter_code = _pick(r, "referred_by_code", "referral_code", "agent_code", "code")
        fake = {
            "full_name": recruiter_name,
            "referral_code": recruiter_code,
            "town": _pick(r, "town", "city", "location"),
            "region": _pick(r, "region"),
            "created_at": _pick(r, "created_at", "registered_at", "signup_date"),
        }
        if recruiter_name:
            row = ensure_agent(fake, forced_name=recruiter_name, source="clients_recruiter")
            if row:
                row["clients_count"] += 1
    ...
    for r in drivers:
        ...
        if recruiter_name:
            row = ensure_agent(fake, forced_name=recruiter_name, source="drivers_recruiter")
            if row:
                row["drivers_count"] += 1

    rows = [r for r in merged.values() if (r.get("full_name") or "").strip()]
    rows.sort(key=lambda r: (str(r.get("full_name") or "").lower(), str(r.get("created_at") or "")), reverse=False)
    return jsonify({"ok": True, "rows": rows})


@app.route("/api/admin/agent_status_update", methods=["POST"])
def api_admin_agent_status_update():
    ...
    if new_status not in ["approved", "rejected", "pending_approval", "blocked"]:
        return jsonify({"ok": False, "error": "Invalid status"}), 400

    updated = False

    for table_name in ["agent_profiles", "agents"]:
        try:
            q = sb_admin.table(table_name).update({"status": new_status})
            if agent_id:
                q = q.eq("id", agent_id)
            elif email:
                q = q.eq("email", email)
    ...
    return jsonify({"ok": True, "updated": updated, "status": new_status})


@app.route("/admin/agents-center")
def admin_agents_center():
    from flask import session, redirect
...
<head>
  <meta charset="utf-8">
  <title>Agent Management Center</title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <style>
    body { font-family: Arial, sans-serif; background:#f6f8fb; margin:0; padding:24px; color:#111827; }
    .wrap { max-width: 1200px; margin: 0 auto; }
    .top { margin-bottom:16px; }
    .top a { text-decoration:none; color:#0b57d0; font-weight:700; }
    .card { background:#fff; border-radius:18px; padding:22px; box-shadow:0 12px 30px rgba(0,0,0,.08); }
    h1 { margin:0 0 8px; font-size:28px; }
    p { margin:0 0 18px; color:#4b5563; }
    .toolbar { display:grid; grid-template-columns: 1fr 220px 180px; gap:12px; margin-bottom:18px; }
    input, select, button { padding:12px 14px; border-radius:12px; border:1px solid #d1d5db; font-size:14px; }
    button { background:#0b57d0; color:#fff; border:none; cursor:pointer; font-weight:700; }
    table { width:100%; border-collapse:collapse; }
    th, td { text-align:left; padding:12px 10px; border-bottom:1px solid #e5e7eb; font-size:14px; vertical-align:top; }
    ...
    .sm { padding:8px 10px; font-size:12px; border-radius:10px; }
    .approve { background:#15803d; }
    .reject { background:#b91c1c; }
    .open { background:#111827; }
    .muted { color:#6b7280; font-size:12px; }
  </style>
</head>
<body>
  <div class="wrap">
    <div class="top"><a href="/dashboard/admin">← Back to Admin Dashboard</a></div>
    <div class="card">
      <h1>Agent Management Center</h1>
      <p>View all agents, recruiter agents, activity counts, timestamps, and open individual profiles.</p>

      <div class="toolbar">
        <input id="searchBox" placeholder="Search by name, email, code, phone...">
...
function profileUrl(a) {
  if (a.id) return `/api/admin/agent_profile/${encodeURIComponent(a.id)}`;
  return "#";
}
  ...
  return `
    <tr>
      ...
      <td>
        ...
        <div class="muted">Code: ${esc(a.referral_code || "-")} • Source: ${esc(a.source_table || "-")}</div>
      </td>
      <td>${esc(a.email || "-")}</td>
      <td>${esc(a.phone || "-")}</td>
      <td>${esc(a.clients_count ?? 0)}</td>
      <td>${esc(a.drivers_count ?? 0)}</td>
      <td>${esc(fmtDate(a.created_at))}</td>
      <td>
        <div class="btns">
          <button class="sm approve" onclick='updateStatus(${JSON.stringify(JSON.stringify({id: a.id || "", email: a.email || "", status:"approved"}))})'>Approve</button>
          <button class="sm reject" onclick='updateStatus(${JSON.stringify(JSON.stringify({id: a.id || "", email: a.email || "", status:"rejected"}))})'>Reject</button>
          <a class="sm open" style="text-decoration:none;color:#fff;" href="/admin/agent-profile/${encodeURIComponent(a.id || "")}" target="_blank">Open Profile</a>
        </div>
      </td>
    </tr>
  `;
...
function render() {
  ...
  const rows = ALL_AGENTS.filter(a => {
    const hay = [
      a.full_name, a.email, a.phone, a.referral_code, a.source_table, a.status
    ].join(" ").toLowerCase();

    const statusOk = !st || String(a.status || "").toLowerCase() === st;
  ...
  if (!rows.length) {
    body.innerHTML = `<tr><td colspan="8">No agents found</td></tr>`;
    return;
  }
...
function loadAgents() {
  fetch("/api/admin/agents_full")
    .then(r => r.json())
    .then(data => {
      ALL_AGENTS = Array.isArray(data) ? data : (data.rows || []);
      render();
    })
    .catch(err => {
      document.getElementById("agentsBody").innerHTML = `<tr><td colspan="8">Failed to load agents</td></tr>`;
      console.error(err);
    });
}
...
function updateStatus(raw) {
  ...
  fetch("/api/admin/agent_status_update", {
    method: "POST",
...
@app.route("/admin/agent-profile/<agent_id>")
...
<head>
  ...
  <style>
    ...
    th {{ background:#f9fafb; }}
    .row {{ display:grid; grid-template-columns:180px 1fr; gap:8px; padding:8px 0; border-bottom:1px solid #f0f2f5; }}
    .label {{ color:#6b7280; font-weight:700; }}
    .btns {{ display:flex; gap:10px; flex-wrap:wrap; margin-top:16px; }}
    button {{ padding:10px 14px; border:none; border-radius:10px; color:#fff; cursor:pointer; font-weight:700; }}
    .approve {{ background:#15803d; }}
    .reject {{ background:#b91c1c; }}
    .refresh {{ background:#111827; }}
  </style>
</head>
<body>
  <div class="wrap">
    <div class="top"><a href="/admin/agents-center">← Back to Agent Management Center</a></div>

    <div class="card" style="margin-top:16px;">
      <h1 id="agentName">Agent Profile</h1>
      <div class="muted" id="agentMeta">Loading...</div>
      <div class="btns">
...
function kv(label, value) {{
  return `<div class="row"><div class="label">${{esc(label)}}</div><div>${{value == null || value === "" ? "-" : esc(value)}}</div></div>`;
}}

function loadProfile() {{
  fetch(`/api/admin/agent_profile/${{encodeURIComponent(AGENT_ID)}}`)
    .then(r => r.json())
    .then(data => {{
      const a = data.agent || {{}};
      ...
      document.getElementById("agentName").textContent = a.full_name || a.name || "Agent Profile";
      document.getElementById("agentMeta").innerHTML = `
        ${{pill(a.status)}} &nbsp; | &nbsp;
        Code: ${{esc(a.referral_code || "-")}} &nbsp; | &nbsp;
        Created: ${{esc(fmtDate(a.created_at))}}
      `;

      document.getElementById("details").innerHTML = [
        kv("Full name", a.full_name || a.name),
        kv("Email", a.email),
        kv("Phone", a.phone || a.phone_number),
        kv("Username", a.username),
        kv("Referral code", a.referral_code),
        kv("Region", a.region),
        kv("Town", a.town),
        kv("Role", a.role),
        kv("Status", a.status),
      ...
      document.getElementById("summary").innerHTML = [
        kv("Wallet balance", data.wallet_balance),
        kv("Clients registered", clients.length),
        kv("Drivers registered", drivers.length),
        kv("Total earnings", a.total_earnings),
        kv("Created at", fmtDate(a.created_at)),
      ].join("");
      ...
      clientsBody.innerHTML = clients.length
        ? clients.map(c => `
            <tr>
              <td>${{esc(c.full_name || c.name || "-")}}</td>
              <td>${{esc(c.phone || c.phone_number || "-")}}</td>
              <td>${{esc(c.status || c.approval_status || "-")}}</td>
              <td>${{esc(fmtDate(c.created_at || c.registered_at || c.signup_date))}}</td>
            </tr>
          `).join("")
        : `<tr><td colspan="4">No clients found</td></tr>`;

      const driversBody = document.getElementById("driversBody");
      driversBody.innerHTML = drivers.length
        ? drivers.map(d => `
            <tr>
              <td>${{esc(d.full_name || d.name || "-")}}</td>
              <td>${{esc(d.phone || d.phone_number || "-")}}</td>
              <td>${{esc(d.status || d.approval_status || "-")}}</td>
              <td>${{esc(fmtDate(d.created_at || d.registered_at || d.signup_date))}}</td>
            </tr>
          `).join("")
        : `<tr><td colspan="4">No drivers found</td></tr>`;
    }})
    .catch(err => {{
      document.getElementById("details").innerHTML = "<div>Failed to load profile</div>";
      console.error(err);
    }});
}}
...
function updateStatus(status) {{
  fetch("/api/admin/agent_status_update", {{
    method: "POST",
    headers: {{"Content-Type":"application/json"}},
    body: JSON.stringify({{agent_id: AGENT_ID, status}})
  }})
  .then(r => r.json())
  .then(() => loadProfile())
  .catch(err => console.error(err));
