...
def api_admin_agents():
    ...
    for row in _safe_table_rows("agent_profiles"):
        ...
        code = _norm(_pick(row, "referral_code", "agent_code", "code", "my_referral_code"))
        key = email or code or _norm(_pick(row, "id"))
        if not key:
            continue

        merged[key] = {
            "id": _pick(row, "id"),
            "full_name": _pick(row, "full_name", "name", "agent_name"),
            "email": _pick(row, "email"),
            "phone": _pick(row, "phone", "mobile", "phone_number"),
            "town": _pick(row, "town", "city", "location"),
            "region": _pick(row, "region"),
            "referral_code": _pick(row, "referral_code", "agent_code", "code", "my_referral_code"),
    ...
    for row in _safe_table_rows("agents"):
        ...
        if key in merged:
            ...
            existing["full_name"] = existing["full_name"] or _pick(row, "full_name", "name", "agent_name")
            ...
            existing["referral_code"] = existing["referral_code"] or _pick(row, "referral_code", "agent_code", "code", "my_referral_code")
            ...
            if existing["source_table"] != "agent_profiles":
                existing["source_table"] = "agents"
        else:
            merged[key] = {
                "id": _pick(row, "id"),
                "full_name": _pick(row, "full_name", "name", "agent_name"),
