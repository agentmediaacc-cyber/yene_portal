...
@app.route('/api/admin/wallet_ledger', methods=['GET'])
def api_admin_wallet_ledger():
    try:
        res = supabase.table('agent_wallet_ledger').select('*').order('created_at', desc=True).execute()
        return jsonify({'success': True, 'rows': res.data or []})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/admin/assign_agent_region/<agent_id>', methods=['POST'])
def api_admin_assign_agent_region(agent_id):
    try:
        data = request.get_json(silent=True) or {}
        region = (data.get('region') or '').strip()
        ...
        if not payload:
            return jsonify({'success': False, 'error': 'No region or town provided'}), 400

        supabase.table('agents').update(payload).eq('id', agent_id).execute()
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500


@app.route('/api/admin/agent_wallet_fund/<agent_id>', methods=['POST'])
def api_admin_agent_wallet_fund(agent_id):
    try:
        data = request.get_json(silent=True) or {}
        amount = float(data.get('amount') or 0)
        note = (data.get('note') or 'Manual wallet funding by admin').strip()

        if amount <= 0:
            return jsonify({'success': False, 'error': 'Amount must be greater than zero'}), 400

        agent_res = supabase.table('agents').select('*').eq('id', agent_id).limit(1).execute()
        ...
        payload = {
            'agent_auth_id': auth_id,
            'week_start': str(week_start),
            'week_end': str(week_end),
            'entry_type': 'credit',
            'amount': amount,
            'reference': 'manual-fund-' + str(uuid4()),
            'note': note,
            'agent_id': agent_id,
            'agent_email': email,
            'txn_type': 'credit',
            'description': note,
            'reference_no': 'manual-fund-' + str(uuid4()),
            'status': 'approved'
        }

        supabase.table('agent_wallet_ledger').insert(payload).execute()
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def api_admin_agent_wallet_bonus(agent_id):
    try:
        data = request.get_json(silent=True) or {}
        amount = float(data.get('amount') or 0)
        note = (data.get('note') or 'Manual bonus by admin').strip()
        ...
        payload = {
            ...
            'entry_type': 'credit',
            'amount': amount,
            'reference': 'manual-bonus-' + str(uuid4()),
            'note': note,
            'agent_id': agent_id,
            'agent_email': email,
            'txn_type': 'credit',
            'description': note,
            'reference_no': 'manual-bonus-' + str(uuid4()),
            'status': 'approved'
        }
...
@app.route('/api/admin/update_client_trips/<client_id>', methods=['POST'])
def api_admin_update_client_trips(client_id):
    try:
        data = request.get_json(silent=True) or {}
        trips_completed = int(data.get('trips_completed') or 0)
...
@app.route('/api/admin/update_driver_trips/<driver_id>', methods=['POST'])
def api_admin_update_driver_trips(driver_id):
    try:
        data = request.get_json(silent=True) or {}
        verified_trips = int(data.get('verified_trips') or 0)
        trips_completed = int(data.get('trips_completed') or verified_trips)
...
# --- ADMIN API: FUNDING, DELETING & GPS TRACKING ---
@app.route('/api/admin/agent_details/<email>')
def get_agent_details(email):
    if not session.get('is_admin'): return jsonify({'error': 'Unauthorized'}), 403
    try:
        # Fetching specific agent details including coordinates from your profiles/activity table
        agent = supabase.table('profiles').select('*').eq('email', email).single().execute()
...
def admin_fund_agent():
    if not session.get('is_admin'): return jsonify({'error': 'Unauthorized'}), 403
    data = request.json
    email, amount = data.get('email'), float(data.get('amount', 0))
    try:
        user_data = supabase.table('profiles').select('wallet_balance').eq('email', email).single().execute()
        new_bal = (user_data.data.get('wallet_balance') or 0) + amount
        supabase.table('profiles').update({'wallet_balance': new_bal}).eq('email', email).execute()
        return jsonify({'success': True, 'message': f'Loaded N to {email}'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
