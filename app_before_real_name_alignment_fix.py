...
def api_admin_weekly_agent_report_pdf():
    ...
    def remember_agent(row):
        ...
        if name:
            name_to_code[_norm(name)] = key
        if code:
            name_to_code[_norm(code)] = key
    ...
    def ensure_bucket(agent_name="", agent_code="", phone=""):
        ...
        if key not in report:
            report[key] = {
                "agent_name": _clean(agent_name) or "Unknown Agent",
            ...
            if agent_name and (not report[key]["agent_name"] or report[key]["agent_name"] == "Unknown Agent"):
                report[key]["agent_name"] = _clean(agent_name)
    ...
    def bucket_for_row(r):
        ...
        if code_key and code_key in agents:
            a = agents[code_key]
            return ensure_bucket(a["name"], a["code"], a["phone"])

        mapped_key = name_to_code.get(name_key)
        if mapped_key and mapped_key in agents:
            a = agents[mapped_key]
            return ensure_bucket(a["name"], a["code"], a["phone"])
